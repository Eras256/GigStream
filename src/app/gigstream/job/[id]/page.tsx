            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                {!isAssigned && (
                  <motion.button
                    ...
                    className="px-4 py-2 bg-gradient-to-r from-somnia-purple to-mx-green rounded-xl text-white font-bold text-sm shadow-neural-glow flex items-center space-x-2"
                  >
              ...
              {showAssignForm && !isAssigned && (
                <motion.div
                  initial={{ opacity: 0, height: 0 }}
                  animate={{ opacity: 1, height: 'auto' }}
                  className="mb-6 backdrop-blur-xl bg-white/5 rounded-2xl p-6 border border-white/10"
                >
                  <div className="mb-4 p-4 bg-somnia-cyan/20 border border-somnia-cyan/30 rounded-xl text-somnia-cyan">
                    <p className="font-bold mb-2 flex items-center space-x-2">
                      <UserPlus className="w-5 h-5" />
                      <span>Assign Worker Directly</span>
                    </p>
                    <p className="text-sm mb-2">Assign a worker without requiring bids. Useful for new workers who don&apos;t have enough reputation yet.</p>
                    <p className="text-xs text-somnia-cyan/80 mb-2">
                      💡 <strong>Tip:</strong> Ask the worker to share their wallet address (0x...), then paste it in the field below.
                    </p>
                    {!isEmployer && (
                      <div className="mt-2 p-2 bg-yellow-500/20 border border-yellow-500/30 rounded-lg text-yellow-400 text-xs">
                  ...
                  <div className="space-y-4">
                    ...
                    <motion.button
                      ...
                      className="w-full px-6 py-3 bg-gradient-to-r from-somnia-cyan to-mx-green rounded-xl text-white font-bold shadow-neural-glow disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isAssigningWorker ? (
                    ...
                    {assignWorkerHash && (
                      <div className="text-center space-y-1">
                        ...
                        <a
                          ...
                          className="text-xs text-somnia-cyan hover:text-somnia-cyan/80 underline"
                        >
                          View on Explorer
                        </a>
                      </div>
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-2 mb-3">
        <History className="w-4 h-4 text-somnia-cyan" />
        <h3 className="text-sm font-bold text-white">Worker History</h3>
      </div>
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div>
          <div className="text-white/60 text-xs mb-1">Reputation</div>
          <div className="text-white font-bold">{repScore} pts</div>
          {repScore < 10 && (
            <div className="text-xs text-yellow-400 mt-1">New worker</div>
          )}
        </div>
        <div>
          <div className="text-white/60 text-xs mb-1">Completed Jobs</div>
          <div className="text-white font-bold">{completedJobs}</div>
        </div>
      </div>
      {completedJobs > 0 && (
