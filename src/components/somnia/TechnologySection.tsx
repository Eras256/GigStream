    ...
    <section id="technology" className="py-16 bg-gradient-to-b from-somnia-dark to-somnia-dark/95 relative overflow-hidden neural-bg">
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        >
          ...
          >
            <Zap className="w-8 h-8 text-somnia-cyan mx-auto" />
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {technologies.map((tech, idx) => (
            ...
            >
              <div className="h-full backdrop-blur-xl bg-white/5 border border-somnia-cyan/20 rounded-3xl p-8 hover:border-somnia-cyan/50 transition-all duration-500 hover:shadow-neural-glow-xl neural-hover relative overflow-hidden group">
                <div className="neural-shimmer absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity" />
                {/* Icon */}
                <motion.div 
                  className={`w-16 h-16 bg-gradient-to-br ${tech.color} rounded-2xl flex items-center justify-center mb-6 shadow-neural-glow relative z-10`}
                ...
                {/* Title */}
                <h3 className="text-2xl font-bold text-white mb-3 relative z-10 group-hover:text-somnia-cyan transition-colors">{tech.title}</h3>

                {/* Metrics Badge */}
                <div className={`inline-block px-3 py-1 bg-gradient-to-r ${tech.color} rounded-full mb-4`}>
                  <span className="text-white font-bold text-sm">{tech.metrics}</span>
                </div>

                {/* Description */}
                <p className="text-white/80 mb-6 leading-relaxed relative z-10">{tech.description}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {['Somnia', 'Solana', 'Ethereum L2s'].map((chain, idx) => (
              <div key={chain} className="text-center">
                <div className={`h-${idx === 0 ? '32' : idx === 1 ? '20' : '12'} bg-gradient-to-t ${idx === 0 ? 'from-[#00D4FF] to-[#7B00FF]' : 'from-white/20 to-white/10'} rounded-t-xl mb-4`} />
                <p className="text-white font-bold">{chain}</p>
